    ...
    for (final controller in controllers) {
      ...
      expect(
        ...
        allOf(
          ...
          isA<HandlerContext>()
              .having((ctx) => ctx.name, 'name', 'event')
              .having(
                (ctx) => ctx.meta['double'],
              ...
              .having(
                ...
                allOf(isNotNull, isA<DateTime>()),
              )
              .having(
                (ctx) => ctx.meta['duration'],
                'meta should contain duration',
                allOf(isNotNull, isA<Duration>(), isNot(Duration.zero)),
              )
              .having((ctx) => ctx.controller, 'controller', same(controller))
              .having((ctx) => ctx.isDone, 'isDone', isTrue),
        ),
      );
      expect(observer.lastErrorContext, isNull);
      expect(Controller.context, isNull);
...
abstract class _FakeControllerBase extends StateController<bool> {
  ...
  Future<void> event({
    ...
    name: 'event',
    meta: {...?meta, 'started_at': DateTime.now()},
  );
}
