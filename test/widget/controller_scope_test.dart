...
void _$valueGroup() => group('ControllerScope.value', () {
  test('constructor', () {
    expect(() => ControllerScope(FakeController.new), returnsNormally);
    expect(ControllerScope(FakeController.new), isA<ControllerScope>());
  });

  testWidgets('inject_and_recive', (tester) async {
...
void _$createGroup() => group('ControllerScope.create', () {
  ...
  testWidgets('inject_and_recive', (tester) async {
    ...
    expect(find.text('0'), findsOneWidget);
    expect(find.text('1'), findsNothing);
    final context = tester.firstElement(
      find.byType(ControllerScope<FakeController>),
    );
...
void _$additionalGroup() => group('ControllerScope.additional', () {
  ...
  testWidgets('maybeOf should return null if no controller is found', (
    ...
    final foundController = ControllerScope.maybeOf<FakeController>(context);
    expect(foundController, isNull);
  });

  testWidgets('maybeOf should return controller if present', (tester) async {
    final controller = FakeController();
    await tester.pumpWidget(
      TestUtil.appContext(
        child: ControllerScope.value(
    ...
    final foundController = ControllerScope.maybeOf<FakeController>(
      context,
      listen: true,
    );
    expect(foundController, equals(controller));
  ...
  testWidgets('_notFoundInheritedWidgetOfExactType should throw error', (
    ...
    await tester.pumpWidget(
      TestUtil.appContext(child: const SizedBox.shrink()),
    );
    await tester.pumpAndSettle();

    final context = tester.firstElement(find.byType(HeroControllerScope));
    expect(() => ControllerScope.of(context), throwsArgumentError);
  });
