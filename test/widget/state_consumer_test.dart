...
void _$baseGroup() => group('base - ', () {
  ...
  testWidgets('should not rebuild when states are identical in _valueChanged', (
    tester,
  ) async {
  ...
  testWidgets('should use child if builder is not provided', (tester) async {
    ...
    await tester.pumpWidget(
      TestUtil.appContext(
        child: ControllerScope.value(
          controller,
          child: StateConsumer(controller: controller, child: childWidget),
        ),
      ),
    );
  ...
  testWidgets('should rebuild with widget child '
        ...
        child: ControllerScope.value(
          ...
          child: StateConsumer(
            ...
            builder: (context, state, child) =>
                Column(children: [Text('State: $state'), child ?? childWidget]),
            child: childWidget,
          ),
        ),
      ),
...
void _$debugFillPropertiesGroup() => group('debugFillProperties - ', () {
  testWidgets('should fill full debug properties correctly', (tester) async {
    final controller = FakeController();
  ...
  testWidgets('should fill debug properties correctly', (tester) async {
    ...
    // Check all expected properties are filled
    final propertyNames = properties.properties.map((p) => p.name).toList();
    expect(propertyNames, containsAll(['Controller', 'State', 'isProcessing']));
  });
});

void _$didUpdateWidgetGroup() => group('didUpdateWidget - ', () {
  testWidgets('should use controller from ControllerScope '
